...
def exercise_translate(request, exercise, language):
    ...
    if usetranslations:
        backup_name = (
            "{:%Y%m%d_%H:%M:%S_%f_}".format(now()) + request.user.username + ".xml"
        )
        ...
        try:
            ...
            messages += exercise_translate_language(
                dbexercise, request.data["xml"], language, action, course_pk
            )
        except IOError as e:
            messages.append(("error", str(e)))
        try:
            messages += Exercise.objects.add_exercise(
                dbexercise.path, dbexercise.course, db=db
            )
...
def notifymissingstring(request, course_pk, language=None): # LEAVE THIS UNUSED ARG FOR LEGACY  FRONTEND
    ...
    try :
        ...
        if dbcourse.use_auto_translation and  not caches['default'].get('temporarily_block_translations' ) :
            ...
            print(f" SKIP TRANSLATION {missingstring} since  use={dbcourse.use_auto_translation}  and  cache={ caches['default'].get('temporarily_block_translations')} ") 
    except Exception as e :
        logger.error(f"Unknown Exception in NOTIFYMISSINGSTRING {type(e).__name__} subdomain={subdomain} user={request.user} data={data} ")
...
def translationdict(request, course_pk, language=None):
    ...
    trans = list(
        Translation.objects.filter(
            exercise__isnull=True, language=language
        ).values_list("altkey", "translated_text")
    )
    ...
    try :
        ...
        for txt in txts:
            ...
            #logger.error(f"FRESH TRANSLATION OF {txt}")
                ...
                txt = re.sub(
                    r"(\$[^\$]*\$)", lhtag + "\\1" + rhtag, txt, flags=re.MULTILINE
                )  # DONT TRANSLATE MATH
                # txt = re.sub(r"(\\\[.*\\\])", lhtag + "\\1" + rhtag, txt,  flags=re.MULTILINE )  # DONT TRANSLATE MATH
                txt = re.sub(
                    r"(\\\[)", lhtag + "\\1", txt, flags=re.MULTILINE
                )  # DONT TRANSLATE MATH
                txt = re.sub(
                    r"(\\\])", "\\1" + rhtag, txt, flags=re.MULTILINE
                )  # DONT TRANSLATE MATH
                txt = re.sub(
                    r"(@\w+)", lhtag + "\\1" + rhtag, txt
                )  # DONT TRANSLATE MACRO VARIABLES
        ...
        if len(txtsin) > 0:
            if not settings.USE_DEEPL :
                ...
                translate_client = translate.Client(credentials=credentials)rigtxt = txt
                translations = translate_client.translate(
                    txtsin, target_language=lang, format_="html"
                )  
            else :
                print(f"NEW TRANSLATION TXTSIN = {lang} {txtsin}")
                translator = deepl.Translator(settings.DEEPL_AUTH_KEY)
                translations = translator.translate_text(txtsin, target_lang=deepl_lang,split_sentences='nonewlines',tag_handling='html')
            #for r in translations :
            #    print(f"{r.text} {r.detected_source_lang}")
            #translations = translate_client.translate(
            #    txtsin, target_language=lang, format_="html"
            #)
        else:
        ...
        for translation in translations:
            if settings.USE_DEEPL :
                translated_text = u"{}".format(translation.text)
            else :
        ...
        for (txtin, txtout) in zipped:
            ...
            tr.save()
        txtsout = []
        for txtin in origtxts:
            if altkey is None:
                tr = Translation.objects.filter(original_text=txtin, language=lang, exercise=exercise)
            else:
                tr = Translation.objects.filter(original_text=txtin, language=lang, altkey=altkey, exercise=None)
            try:
                tr = tr[0]
                txtsout.append(tr.translated_text)
            except Exception as e:
                logger.error(
                    "UNTRANSLATED %s FOR UNKNOWN REASON  %s %s "
                    % (tr.original_text, type(e).__name__, str(e))
                )
...
def translate_xml(xml, lang, action, course_pk, dbexercise, domakepretty):
    ...
    if "translate" in action:
        ...
        newxml = translate_xml_language(
            xml, [lang], course_pk, dbexercise, domakepretty
        )
        ...
        try:
            ...
            logger.error(
                "TRANSLATIONS ERROR IN remove %s , %s ", (str(e), formatted_lines)
            )
...
def translate_xml_language(xml, langs, course_pk, dbexercise, domakepretty):
    ...
    for elem in elems_already_translated:
        ...
        objs = Translation.objects.filter(
            language=lang, original_text=original_txt, exercise=dbexercise
        ).union(
            Translation.objects.filter(
                language=lang, original_text=original_txt, exercise__isnull=True
            )
        )
        if objs.exists():
            for obj in objs:
    ...
    for lang in langs:
        ...
        originals_in_database = set(
            Translation.objects.filter(language=lang, exercise=dbexercise).values_list(
                "original_text", flat=True
            )
        )
...
def remove_language(xml, lang, course_pk):
    ...
    try:
        ...
        formatted_lines = traceback.format_exc()
        logger.error(
            "TRANSLATIONS ERROR IN remove_language %s , %s ", (str(e), formatted_lines)
        )
        return xmlorig
