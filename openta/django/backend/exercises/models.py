...
def validate_question_xml(  db , exercise_key, question_key, username, xml ):
    ...
    #print(f"QUESTION_XMLTREE_IN_MODEL {xml}")
    #xml = etree.tostring( global_xmltree)
    #print(f"GLOBAL_XMLTREE_IN_MODEL {xml}")
...
class ExerciseManager(models.Manager):
    ...
    def add_exercise_full_path(self, path, course, db=settings.DB_NAME):
        ...
        """
        print(f"ADD_EXERCISE_FULL_PATH")
    ...
    def add_exercise(self, exercise_path, course, db=settings.DB_NAME):
        ...
        """
        print(f"ADD_EXERCISE_RELATIVE_PATH")
        progress = []
        translation_name = {}
        ...
        try :
            ...
            #logger.error(f"course= {course}")
            #logger.error(f"USE_AUTO_TRANSLATION = {course.use_auto_translation}")
            #logger.error(f" DB={db} KEY={key} NAME={name} PATH={exercise_path} FOLDER={folder}")
            ...
            dbexercise.translated_name = json.dumps( translation_name )
            dbexercise.save();
            print(f"FINISHED A SAVE of {name}")
        except Exception as e :
            print(f"GOING TO DEFAULTS {str(e)} ")
            defaults = dict(
                name=name,
                translated_name=JSON.dumps(translation_name),
                path=exercise_path,
