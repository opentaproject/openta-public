...
class SecondDevLinearAlgebraTest(OpenTAStaticLiveServerTestCase):
    ...
    def first_exercise(self):
        ...
        #    logger.error("exercise elements = %s", element.text )
        #
    ...
    def answer(self):
        ...
        logger.error("Looking for orange forbidden c ")
        #wait.until(EC.presence_of_element_located((By.XPATH, "//span[contains(@style,'color:orange') and text()='c']")))
        #WebDriverWait(sel,350000).until(EC.presence_of_element_located((By.XPATH, "//div[contains(@class,'uk-alert-warning') and text()[contains(.,\'c\')] ]")))
        logger.error("Forbidden c  found")
        logger.error("ALL OK")
        wait.until(EC.presence_of_element_located((By.XPATH, "//div[contains(.,'Not valid')]")))
    ...
    def test_1_dev_linear_algebra(self):
        ...
        """
        logger.error("OPEN SITE DEV3")
        self.open_site()
        logger.error("OPEN OPTIONS CHANGE")
        self.change_exercise_options()
        logger.error("LOGIn")
        self.login()
        logger.error("FIRST")
        self.first_exercise()
        logger.error("ANSWSER")
        self.answer()
        logger.error("LOGOUT")
        self.logout()
