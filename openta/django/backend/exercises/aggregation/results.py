...
def students_results(cache_seconds=settings.STATISTICS_CACHE_TIMEOUT, force=False, task=None, course=None):
    ...
    antimigratekey = f"queutask-has-perm-{task.pk}"  ## HACK AVOID MIGRATION WITH FOREIGN KEY
    ...
    result = calculate_students_results(course=course, task=task)
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    result.append({"date": now})
    if cachekey:
        # print(f" SET CACHEKEY {cachekey}")
...
def calculate_students_results(course, task=None):
    logger.error("CALCULATE StuDENTS RESULTS")
    if task == None:
    ...
    if settings.MULTICOURSE and not settings.RUNTESTS:
        ...
        db = "default"
    students = (
        User.objects.using(db).filter(groups__name="Student", opentauser__courses=course).exclude(username="student")
    )
    results = []
    n_students = students.count()
    ...
    for index, student in enumerate(students):
        if task is not None:
            ...
            task.progress = round(((index + 1) / n_students) * 100)
            task.save(using='default')
        summary = calculate_unsafe_user_summary(student.pk, course.id, subdomain, None)
        results.append(summary)
...
def calculate_unsafe_user_summary(user_pk, course_pk, subdomain, dbexercises):
    logger.debug("UNSAFE GLOBALS SUBDOMAIN = %s DB_NAME=  %s " % (settings.SUBDOMAIN, settings.DB_NAME))
    ...
    sums = get_unsafe_exercise_summary(user_pk, course_pk, dbexercises, db)
