...
from exercises.modelhelpers import exercise_test, serialize_exercise_with_question_data
...
def get_unsafe_exercise_summary(user, course_pk, dbexercises, db):
...
def get_folder_etag(request, course_pk, fstring):
    ...
    (cache, cachekey) = get_cache_and_key(
        "user_exercise_tree:", userPk=str(user.pk), coursePk=course_pk, subdomain=subdomain, exercise_filter=fstring
    )
...
def exercise_set_selected_exercises(request, instruction):
    settings.SUBDOMAIN = get_subdomain(request)
    subdomain, db = get_subdomain_and_db(request)
    if "selectedExercises" in request.data.keys():
        request.session["selectedExercises"] = request.data["selectedExercises"]
    else:
        return Response({})
