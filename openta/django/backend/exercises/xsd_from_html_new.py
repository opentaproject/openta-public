...
def extract_rows(doc: str) -> list[tuple[str, str]]:
    ...
    ndoc = lhtml.fromstring(doc)
    rows = []
    i = 0;
    for tr in ndoc.xpath('//tr') :
        tds = tr.xpath('./td')
        if len(tds) >= 2 and i > 0 :
            c1 =tds[0].text_content().strip()
            c2= tds[1].text_content().strip()
            rows.append((c1, c2))
        i +=1 ;
    return rows
...
def main() -> int:
    args = parse_args()
    try:
        xml = Path(args.html_file).read_text(encoding="utf-8")
    except Exception as e:
        print(f"error: failed to read {args.html_file}: {e}", file=sys.stderr)
        return 2
    root = lhtml.fromstring( xml.encode() )
    for style in root.xpath('//style'):
        style.getparent().remove(style)
    for t in ['//span','//p','//br','b'] :
        for span in root.xpath(t) :
            span.drop_tag()
    for a in ['class','valign'] :
        for el in root.xpath(f'//*[@{a}]'):   # all elements with a class attribute
            del el.attrib[a]   
