...
def import_server(import_zip_path, subdomain, merge=False, legacy=False):
    ...
    zipfile = f"/tmp/{subdomain}.zip"
    logger.error(f"ZIPFILE = {zipfile}")
    shutil.copy2(import_zip_path,zipfile)
    logger.error(f"COPY ")
    result = subprocess.run(['./db_mergecourse_from_zip', zipfile,  f"{subdomain}" ] , capture_output=True, text=True)
    subprocess.run(['touch',f'/subdomain-data/{subdomain}/reload']) # UPON LOGOUT LATER, RESET THE DATABASE AND CACHE
    txt = result.stderr
    logger.error(f"TXT = {txt}")
    if  txt :
        txt = txt.split('ERROR:')[1]
        assert False, txt
        yield txt , 0.9 
    else :
        yield "DONE", 1
