...
def health(request, subdomain="", username="", **kwargs):
    logger.error(f"GET HEALT {subdomain} {username} {kwargs}")
...
def login(request, course_name=None):
    ...
    try:
        ...
        messages.add_message(request, messages.ERROR, f'Database "%s" is not configured. {settings.DB_NAME}')
        return render(request, "base_failed.html")
    ...
    if not getattr(request, "limited", False) or settings.RUNNING_DEVSERVER:
        ...
        if cookies == None and not settings.RUNNING_DEVSERVER and not headers.get("Sec-Fetch-Dest", None) == "document":
            ...
            if "Safari" in agent:
                ...
                response = csrf_failure(
                    request,
                    f"CSRF cookie is not set; make sure third-party cookies are allowed, at least from [.*]instructure.com or canvas.gu.se.   ",
                )
...
def frontend_log(request):
    ...
    logger.error('FRONTEND_ERROR frontend_log: %s %s', message, context)
