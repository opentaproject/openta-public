...
def create_database(password="pw", course_key=None, course_name="Test course"):
    ...
    u5 = create_user("admin5", "admin5@test.se", "admin5@test.se", course=course)
...
class OpenTAStaticLiveServerTestCase(StaticLiveServerTestCase):
    ...
    def login(self, username="admin3", pw="pw", assert_role="admin"):
        sel = self.selenium
        wait = WebDriverWait(sel, 2000000 ) # settings.LONG_WAIT)
    ...
    def find_exercise_element(self, exercise):
        ...
        if hasattr(exercise, "exercise_key"):
            ...
            logger.error("RETURNING COURSE-EXERCISE-ITEM")
            logger.error(f"RETURNING COURSE-EXERCISE-ITEM  li.course-exercise-item[id={exercise_key}]")
            #element = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, f"li.course-exercise-item[id={exercise_key}]")))
            element = wait.until(EC.presence_of_element_located((By.ID, exercise_key)) )
            #element = sel.find_element(By.XPATH, f"//li[@id={exercise_key}]")
            logger.error(f"RETURN ELEMENT {element}")
            #clicker = element.find_element(By.XPATH,'.//a')
            #logger.error(f"CLICKER = {clicker}")
            #element = wait.until(EC.element_to_be_clickable((By.XPATH, '//a[@id="course-exercise-item-button"]')))
            atag = element.find_element(By.TAG_NAME,"a")
            return atag
        logger.error("RETURNING COURSE-EXERCISE-BUTTON")
        element = wait.until(EC.element_to_be_clickable((By.XPATH, '//a[@id="course-exercise-item-button"]')))
        return element
    ...
    def answerall(self, exercise, answerdict=None):
        ...
        assert check == 0
        logger.error(f"vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv\nRET = {ret}")
