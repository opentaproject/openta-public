...
import { keyIn } from '../immutablehelpers.js';
import _ from 'lodash';
import immutable from 'immutable';
import { jsonfetch, CSRF_TOKEN, sendFrontendLog } from '../fetch_backend.js';
import { SUBPATH } from '../settings.js';

import { updateAudits, updatePendingStateIn, setAuditExerciseStats } from '../actions.js';

import {} from '../fetchers';
...
function deleteAuditResponseFile(auditResponseId) {
  return (dispatch) => {
    ...
    return jsonfetch('/auditresponsefile/delete/' + auditResponseId + '/', fetchconfig)
      ...
      .catch((err) => {
        console.dir(err);
        sendFrontendLog('error', 'deleteAuditResponseFile failed', { auditResponseId, error: String(err) });
      });
  };
}
...
function fetchUnsentAudits() {
  return (dispatch) => {
    ...
    return jsonfetch('/audit/unsent/')
      ...
      .catch((err) => {
        console.log(err);
        sendFrontendLog('error', 'fetchUnsentAudits failed', { error: String(err) });
      });
  };
}
...
function fetchCurrentAuditsExercise() {
  return (dispatch, getState) => {
    ...
    return jsonfetch('/audit/get/exercise/' + exercise)
      ...
      .catch((err) => {
        console.log(err);
        sendFrontendLog('error', 'fetchCurrentAuditsExercise failed', { error: String(err) });
      });
  };
}
...
function fetchAuditExerciseStats() {
  return (dispatch, getState) => {
    ...
    return jsonfetch('/audit/stats/exercise/' + exercise)
      ...
      .catch((err) => {
        console.log(err);
        sendFrontendLog('error', 'fetchAuditExerciseStats failed', { error: String(err) });
      });
  };
}
...
function sendAudit(auditPk, bcc) {
  return (dispatch) => {
    ...
    return jsonfetch('/audit/send/' + auditPk + '/', fetchconfig)
      ...
      .catch((err) => {
        console.dir(err);
        sendFrontendLog('error', 'sendAudit failed', { auditPk, bcc, error: String(err) });
      });
  };
}
...
function publishReadyAudits(exercise) {
  return (dispatch) => {
    ...
    return jsonfetch('/audit/publishready/' + exercise + '/', fetchconfig)
      ...
      .catch((err) => {
        console.dir(err);
        sendFrontendLog('error', 'publishReadyAudits failed', { exercise, error: String(err) });
      });
  };
}
...
function deleteAudit(auditPk) {
  return (dispatch) => {
    ...
    return jsonfetch('/audit/delete/' + auditPk + '/', fetchconfig)
      ...
      .catch((err) => {
        console.dir(err);
        sendFrontendLog('error', 'deleteAudit failed', { auditPk, error: String(err) });
      });
  };
}
...
function saveAudit(auditPk, auditData) {
  return (dispatch) => {
    ...
    return jsonfetch('/audit/update/' + auditPk + '/', fetchconfig)
      ...
      .catch((err) => {
        console.dir(err);
        sendFrontendLog('error', 'saveAudit failed', { auditPk, error: String(err) });
      });
  };
}
...
function updateAuditStudent(auditPk, updated) {
  return (dispatch) => {
    ...
    return jsonfetch('/audit/update_student/' + auditPk + '/', fetchconfig)
      ...
      .catch((err) => {
        console.dir(err);
        sendFrontendLog('error', 'updateAuditStudent failed', { auditPk, error: String(err) });
      });
  };
}

function fetchNewAudit(exercise, heap = 'fromReady', n_audits) {
  return (dispatch) => {
    dispatch(updatePendingStateIn(['audit', 'newAudit'], true));
    return jsonfetch('/audit/new/exercise/' + heap + '/' + exercise + '/' + n_audits)
      .then(() => dispatch(fetchCurrentAuditsExercise()))
      .then(() => dispatch(updatePendingStateIn(['audit', 'newAudit'], false)))
      .catch((err) => {
        console.log(err);
        sendFrontendLog('error', 'fetchNewAudit failed', { exercise, heap, n_audits, error: String(err) });
      });
  };
}

function fetchNewAuditByUser(exercise, username) {
  return (dispatch) => {
    dispatch(updatePendingStateIn(['audit', 'newAudit'], true));
    return jsonfetch('/audit/newbyuser/exercise/' + username + '/' + exercise + '/')
      .then(() => dispatch(fetchCurrentAuditsExercise()))
      .then(() => dispatch(updatePendingStateIn(['audit', 'newAudit'], false)))
      .catch((err) => {
        console.warn('Failed to create audit by user', { exercise, username }, err);
        sendFrontendLog('error', 'fetchNewAuditByUser failed', { exercise, username, error: String(err) });
      });
  };
}
