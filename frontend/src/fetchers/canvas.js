...
import { updatePendingStateIn, updateTask } from '../actions.js';

import { fetchTaskProgress } from '../fetchers.js';

function uploadProgress(dispatch, evt, courseKey) {
  if (evt.loaded && evt.total && evt.total > 0) {
    return dispatch(
      updatePendingStateIn(['course', courseKey, 'canvasGradebookUploadProgress'], evt.loaded / evt.total)
    );
  } else if (evt.position && evt.totalSize && evt.totalSize > 0) {
    return dispatch(
      updatePendingStateIn(['course', courseKey, 'canvasGradebookUploadProgress'], evt.position / evt.totalSize)
    );
  }
}
