...
const loadExerciseTreeUI = () => {
  try {
    const raw = localStorage.getItem('exerciseTreeUI');
    if (!raw) return {};
...
var defaultState = immutable.fromJS({
  ...
  activityRange: 'all',
  exerciseState: {},
  exerciseTreeUI: loadExerciseTreeUI(),
  results: {
    studentResults: [],
    regradeResults: [],
    analyzeResults: [],
...
export default (state = defaultState, action) => {
  switch (action.type) {
    case 'UPDATE_TRANSLATIONS':
      ...
      var newState = state.mergeDeepIn([], immutable.fromJS(action.json));
      return newState;
    case 'UPDATE_SELECTED_EXERCISES_META':
      return state.setIn(['selectedExercisesMeta'], action.data);
    case 'RESET_SELECTED_EXERCISES':
      return state.setIn(['selectedExercises'], []);
    case 'UPDATE_SELECTED_EXERCISES':
      const present = state.getIn(['selectedExercises'], []);
      const index = present.indexOf(action.exerciseKey);
      if (index > -1) {
        present.splice(index, 1);
      } else {
        present.push(action.exerciseKey);
      }
      return state.setIn(['selectedExercises'], present);
    ...
    case 'UPDATE_EXERCISE_STATE':
      ...
      if (updatedState.hasIn(['audit', 'responsefiles'])) {
        newState = newState.setIn(
          ['exerciseState', action.exercise, 'audit', 'responsefiles'],
          updatedState.getIn(['audit', 'responsefiles'])
        );
      }
    ...
    case 'UPDATE_QUESTION_RESPONSE':
      return state.setIn(
        ['exerciseState', action.exercise, 'question', action.question, 'response'],
    ...
    case 'SET_IMAGE_ANSWERS_DATA':
      return state.setIn(
        ['exerciseState', action.exercise, 'image_answers_data'],
        immutable.fromJS(action.imageAnswersData)
      );
    case 'UPDATE_AGGREGATE_STATISTICS':
      return state.mergeDeepIn(['statistics', 'aggregates'], immutable.fromJS(action.aggregates));
    case 'UPDATE_COURSE_AGGREGATE_STATISTICS':
      return state.setIn(['statistics', 'course_aggregates'], immutable.fromJS(action.course_aggregates));
    case 'UPDATE_EXERCISE_STATISTICS':
      if (!(action.stats == null)) {
        localStorage.setItem('userStats', JSON.stringify(action.stats));
      }
      var fromcache = JSON.parse(localStorage.getItem('userStats'));
      if (fromcache == null) {
        return state;
      } else {
        return state.mergeDeepIn(['exerciseState'], immutable.fromJS(fromcache));
      }
    case 'UPDATE_STUDENT_RESULTS':
      if (!(action.results == null)) {
        localStorage.setItem('userResults', JSON.stringify(action.results));
      }
      var fromcache = JSON.parse(localStorage.getItem('userResults'));
      if (fromcache == null) {
        return state;
      } else {
        return state.setIn(['results', 'studentResults'], immutable.fromJS(fromcache));
      }
    case 'SET_EXERCISE_RECENT_RESULTS':
      return state.setIn(['results', 'exercises', action.exercise, 'recent'], immutable.fromJS(action.data));

    case 'SET_EXERCISE_ANALYZE_RESULTS':
      state.setIn(['results', 'analyzeResults', action.exercise], immutable.fromJS(action.results));
      return state.setIn(['results', 'exercises', action.exercise, 'analyze'], immutable.fromJS(action.data));
    ...
    case 'UPDATE_ANALYZE_RESULTS':
      return state.setIn(['results', 'exercises', action.exercise, 'analyze'], immutable.fromJS(action.results));
    ...
    case 'UPDATE_EXERCISES_VALIDATE_MESSAGES':
      return state.setIn(['exercisesValidateMessages'], immutable.fromJS(action.messages));
    case 'UPDATE_AUDITS':
    ...
    case 'SET_ACTIVITY_RANGE':
      ...
      cookies.set('activity_range', action.range.toString(), { path: '/', secure: true , sameSite: 'none' });
