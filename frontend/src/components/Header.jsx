...
import { SUBPATH, use_sidecar ,sidecar_url, source_url, source_health_check } from '../settings.js';
...
const BaseHeader = ({
  ...
  useEffect(() => {
    if (!source_health_check || !source_url) return;
    try {
      ...
      if (u.hostname.includes('github.com')) {
        ...
        if (parts.length >= 2) {
          ...
          fetch(api, { method: 'GET' })
            .then((res) => setSourceAvailable(res.ok))
            .catch(() => setSourceAvailable(false));
          return;
        }
      }
      // Fallback: if not a supported host, do not hide the link
  ...
  if (!authenticated) {
    return (
      <div className="uk-text uk-margin-small-left uk-align-middle">
        ...
        {source_url && (!source_health_check || sourceAvailable) && (
          <a
            className="uk-margin-small-left uk-text-size"
            href={source_url}
            target="_blank"
  ...
  return (
    <div className="border-top ">
          ...
          <div className="uk-hidden-small uk-align-right">
            {authenticated && !iframed && (
              <div className="uk-padding-large">
                ...
                {source_url && (!source_health_check || sourceAvailable) && (
                  <a className="uk-margin-right" title="Source code" href={source_url} target="_blank" rel="noopener noreferrer">
                    <span className="uk-text-small">Source</span>
                  </a>
                )}
                <a className="uk-margin-right" href="#about-legal" data-uk-modal title="About / Legal">
                  <span className="uk-text-small">About</span>
                </a>
                <a title="Logga ut" href={SUBPATH + '/logout/' + course_name + '/'}>
      ...
      {/* About / Legal modal for AGPL notice */}
      <div id="about-legal" className="uk-modal">
        <div className="uk-modal-dialog">
          <a className="uk-modal-close uk-close"></a>
          <h3>About / Legal</h3>
          <p>
            Copyright (C) 2018-2025 Stellan Östlund and Hampus
            Linander
          </p>
          <p>
            Licensed under the GNU Affero General Public License v3.0 or
            later (AGPL-3.0-or-later). This program is provided without
            any warranty; without even the implied warranty of
            merchantability or fitness for a particular purpose.
          </p>
          <p>
            <a href="https://www.gnu.org/licenses/agpl-3.0.html" target="_blank" rel="noopener noreferrer">
              Read the AGPLv3 License
            </a>
          </p>
          {source_url && (!source_health_check || sourceAvailable) && (
            <p>
