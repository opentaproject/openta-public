...
import { fetchNewAuditByUser } from '../fetchers.js';
  ...
  if (notActive) {
    ...
    var rendernotActive = notActive.map((item, index) => (
      <tr key={'r' + index}>
        <td className="uk-text-right">{index + 1}</td>
        <td>
          <button onClick={() => onAddAudit(activeExercise, item)} className="uk-text-right uk-button-link">
            {item}
          </button>{' '}
        </td>
      </tr>
    ...
    <div>
      <div className="uk-button-group uk-flex uk-flex-column">
        <span data-uk-dropdown="{mode:'click'}">
          <span
            className="uk-button uk-width-1-1 uk-button-small uk-margin-small-top uk-button-primary"
            title="Click for more statistics"
          >
...
const mapDispatchToProps = (dispatch) => ({
  onAddAudit: (exercise, username) => {
    //console.log("username = ", username )
    dispatch(fetchNewAuditByUser(exercise, username));
  }
});
