...
function generateItem(onClickFunc, exercise, activeExercise, exerciseState, meta, showStatistics) {
  ...
  var exercisename = 'C' + exercise.get('name','NONE') + 'C'
  return (
    <li
      className={exercise.get('exercise_key') === activeExercise ? 'uk-active' : ''}
      key={exercise.get('exercise_key')}
    >
      <a
        className={meta.get('published', false) ? '' : 'exercise-unpublished'}
        onClick={() =>
          onExerciseClick(
            exercise.get('exercise_key'),
            exerciseState.getIn([exercise.get('exercise_key'), 'json'], immutable.Map({})).isEmpty()
          )
        }
      >
        <ul>
            ...
            <div className="exercise-list-thumb-wrap">
              <div className="exercise-thumb-badge">
                ...
                {exerciseState.getIn([exercise.get('exercise_key'), 'audit', 'published'], false) && (
                  <Badge
                    type={
                      exerciseState.getIn([exercise.get('exercise_key'), 'audit', 'revision_needed'], false)
                        ? 'error'
                        : 'success'
                    }
                    className={'uk-badge-notification'}
                  >
                    <T>Reviewed</T>
                  </Badge>
                )}
	     ...
	     { thumbnail && ( 
              <SafeImg
                ...
                src={SUBPATH + '/exercise/' + exercise.get('exercise_key') + '/asset/' + thumbnail}
              ></SafeImg>
	     )} 
            </div>
          </li>
  ...
  <div className="uk-text-center " id="exercises-menu">
        ...
        <ul className="uk-nav uk-nav-offcanvas">
          ...
          {exerciselist.map((exercise) =>
            generateItem(
              ...
              activeExercise,
              exerciseState,
              exercise.get('meta') || immutable.Map({}),
              showStatistics
            )
          )}
        </ul>
    ...
    {showOnCanvas && (
      <ul className="uk-nav uk-nav-side uk-list-space exercise-menu exercise-list-on-canvas">
        <li className="uk-nav-header" key="header">
          <a onClick={(ev) => onBack(coursePk)}>
            <i className="uk-icon uk-icon-medium uk-icon-arrow-left"></i>
          </a>{' '}
          <span className="uk-text-large">{folder.split('.')[0]}</span>
        </li>
