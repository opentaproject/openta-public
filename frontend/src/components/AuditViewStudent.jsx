...
import { fetchExerciseRemoteState, updateAuditStudent } from '../fetchers.js';
import { updatePendingStateIn } from '../actions.js';

const BaseAuditViewStudent = ({
  activeExercise,
  hasAuditData,
  auditPk,
  auditor,
  ...
  var renderAuditFiles = auditFiles.map((auditResponse) => (
  ...
  if (hasAuditData) {
    var firstname = auditor.get('first_name');
  }
  var lastname = auditor.get('last_name');
  if (firstname + lastname == '') {
    firstname = auditor.get('email');
  }
  if (firstname + lastname == '') {
    firstname = auditor.get('username');
  }
  if (revisionNeeded) {
    var rclass = 'uk-panel uk-width-1-1 uk-margin-small-left uk-padding-remove revision-needed';
  } else {
    var rclass = 'uk-panel uk-width-1-1 uk-margin-small-left uk-padding-remove revision-not-needed';
  }
      ...
      <div className="uk-container">
        <div className="uk-flex uk-margin-bottom">
          <div className="uk-width-2-3">
            <div className="uk-flex">
              <div>
                <h3 className="uk-margin-small-bottom">
                  Reviewed:
                  {!revisionNeeded && (
                    <Badge className="uk-margin-left">
                      <i className="uk-icon uk-icon-medium uk-icon-check" />
                      <span id="revision-not-needed" /> Final comments{' '}
                    </Badge>
                  )}
                  {revisionNeeded && (
                    <Badge type="error" className="uk-margin-left">
                      <span id="revision-needed" /> Return required for pass
                    </Badge>
                  )}
                </h3>
                ...
                <div className="uk-margin-left uk-flex uk-flex-column">
                  <div>
                    <a
                      className="uk-button"
                      data-uk-tooltip
                      title="Klicka här när du uppdaterat din lösning."
                      onClick={(e) => onUpdated(activeExercise, auditPk, !currentlyUpdated)}
                      id="revision-update"
                    >
                      Uppdaterad
                      {!currentlyUpdated && (
                        <i className="uk-margin-small-left uk-icon uk-icon-small uk-icon-square-o" />
                      )}
                      {currentlyUpdated && (
                        <i className="uk-text-success uk-margin-small-left uk-icon uk-icon-small uk-icon-check-square-o" />
                      )}
                  ...
                  {currentlyUpdated && (
                    <div>
                      <span className="uk-text-small" id="revision-updated">
                        (Granskare uppmärksammad)
                      </span>
                    </div>
                  )}
                </div>
            ...
            <hr />
            <h4>{subject}</h4>
            {points && <h4 className="uk-margin-small-bottom"> StudentPoints: {points} </h4>}
            <div id="audit-message">{message}</div>
          </div>
          {renderAuditFiles.size > 0 && (
            <div className="uk-margin-left">
              <h3>Filer</h3>
              <ul className="uk-subnav uk-subnav-line">{renderAuditFiles}</ul>
            </div>
          )}
        </div>
      </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    onUpdated: (exercise, auditPk, updated) => {
      ...
      dispatch(updateAuditStudent(auditPk, updated)).then((json) => {
        if (!json.error) {
