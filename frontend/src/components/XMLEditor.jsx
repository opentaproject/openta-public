...
import { handleSave, handleReset } from './LoginInfo';
...
var Tools = ({
  ...
  <div className="uk-button-group">
    ...
    {!use_auto_translation && (
      <span
        className="uk-button uk-button-small uk-text uk-text-warning "
        title={'translations must be enabled in admin'}
        data-uk-tooltip
      >
        TRANSLATE OFF
      </span>
  ...
  render() {
    ...
    var savereset = (
      <Tools
        ...
        savesuccess={!modified && saveError === false}
        showreset={modified}
        saveerror={saveError}
        resetpending={resetPending}
        onsave={(event) => this.props.onSave(activeExercise, subdomain)}
        onreset={(event) => onReset(activeExercise)}
    ...
    var use_auto_translation = this.props.use_auto_translation && this.props.languages.size > 0;
    return (
      <div
        className="uk-panel uk-panel-box uk-margin-small-top  uk-margin-small-right"
        style={{ height: '80vh', width: '100%' }}
      >
        {doshowtools && (
          <a className={'uk-button uk-button-small  '} onClick={this.theme_toggle}>
            Switch theme {options.theme}
          </a>
        )}
        {doshowtools && (
          <a className={'uk-button uk-button-small '} onClick={this.keyMap_toggle}>
            Switch keymap {options.keyMap}
          </a>
        )}
        {doshowtools && (
          <a className={'uk-button uk-button-small  '} onClick={this.prettify}>
            {' '}
            Prettify{' '}
          </a>
        )}
        ...
        <Codemirror
          ...
          editorDidConfigure={this.editorDidConfigure}
