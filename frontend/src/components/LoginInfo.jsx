...
import { fetchExerciseJSON, fetchExerciseXML, fetchExerciseRemoteState, fetchSameFolder, saveExercise, resetExercise } from '../fetchers.js';
...
import { SUBPATH, adminurl, student_pk } from '../settings.js';
...
var Tools = ({ showsave, onsave, savepending, savesuccess, saveerror, showreset, resetpending, onreset, selected }) => {
  //console.log("Tools selected = ", selected)
  return (
    <div className="uk-button-group">
      {showsave && (
        <a
          className={'uk-button uk-button-small ' + (saveerror ? 'uk-button-danger' : 'uk-button-success')}
          onClick={onsave}
        >
          Save {savepending ? <i className="uk-icon-cog uk-icon-spin"></i> : <i className="uk-icon-floppy-o"></i>}{' '}
        </a>
      )}
      {showreset && savepending !== true && (
        <a
        ...
        >
          {' '}
          {resetpending ? <i className="uk-icon-cog uk-icon-spin"></i> : <i className="uk-icon-undo"></i>}
        </a>
      )}
      <ExerciseHistory />
      <DeleteExercise />
...
const BaseLoginInfo = ({
  ...
  var renderGroupIcons = groups.map((group) => {
    if (group in groupIcons) {
      return (
        <i
          key={group}
          className={'uk-icon uk-text-success uk-margin-small-left ' + groupIcons[group].icon}
          title={groupIcons[group].alt}
        />
      );
    } else {
  ...
  var renderPending = pendingPaths.map((item) => {
    return (
      pendingState.getIn(item.path, false) && (
        <span key={item.path}>
          {item.name}
          <Spinner icon="uk-icon-bar-chart" size="" className="uk-margin-small-left" />
        </span>
      )
    );
  });
  var my_student = 'student-' + String(user_pk);
  var studentViewDOM =
    author || viewer ? (
      <button
        onClick={(e) =>
          UIkit.modal.confirm('View site as ' + my_student + ' ?', () =>
            window.open(SUBPATH + '/hijack/username/' + my_student + '/', '_self')
          )
        }
        className="uk-button uk-button-mini uk-alert-warning"
        data-uk-tooltip
        title="Log in as default student"
      >
        Student view {my_student}{' '}
      </button>
    ) : (
      ''
    );
  ...
  // console.log("LOGIN_INFO RENDERING")
  //var ihtml = window.document.getElementsByName("csrfmiddlewaretoken")
  //console.log("ihtml=", ihtml)
  //console.log("CSRF_TOKEN=", CSRF_TOKEN)
  //if ( ihtml[0] ) { ihtml = ihtml[0] ;
  //	 	var token = ihtml.getAttribute('value')
  //	} else {
  //	console.log("ihtml NOT DEFINED = ", ihtml)
  //	var token = 'abcegh'
  //	      }
  ...
  var studentViewDOM = (
    <form className="uk-form" action="/hijack/acquire/" method="post" style={{ display: 'inline' }}>
      ...
      <button type="submit">view as student-{String(user_pk)} </button>
  ...
  //console.log("SUBAMINA1 = ", subdomain)
  var adminurlstring = adminurl.toString();
  var coursepk = activeCourse.toString();
  var adminuser = fix_trailing_slash( SUBPATH + '/' + adminurlstring + 'auth/user/');
  var adminhome = SUBPATH + '/' + adminurlstring;
  // var admintranslate = SUBPATH + '/' + adminurlstring + 'translations/translation/';
  var admintranslate = SUBPATH + '/course/' + coursepk + '/translation/edit';
  var aa = admin;
  return (
    <div id="login" className="uk-nav uk-navbar-attached ta-nav border-top">
      <div className="uk-container uk-container-center" style={{ maxWidth: 'none' }}>
        ...
        <div className="uk-navbar-content uk-flex uk-flex-wrap">
          ...
          {activeExercise && menuPositionUnder(menuPath, ['activeExercise']) && (
            <div className="uk-navbar-content uk-padding-remove">
              <a
                href="#offcanvas-exercise-list"
                className="uk-navbar-toggle exercise-list-off-canvas uk-padding-remove"
                data-uk-offcanvas
              />
            </div>
          )}
          <ul className="uk-navbar-nav exercise-list-on-canvas">
            <li>
          ...
          {author && (
            <button className="uk-button uk-margin-small-top uk-hidden-large" data-uk-toggle="{target:'.smallHidden'}">
              <i className="uk-icon-chevron-circle-down" />
            </button>
          )}
          {/* Username */}
          ...
          <div className="uk-navbar-flip">
            <ul className="uk-navbar-nav">
              ...
              {!author && (
                <li>
                  <a
                    href="https://openta-development.github.io/info/html/student/"
                    data-uk-tooltip
                    title={'ReadTheDocs'}
                  >
                    <span className="uk-icon-info"></span>
                  </a>
                </li>
              )}
              ...
              {author && (
                <li>
                  <a href="https://openta-development.github.io/info/" data-uk-tooltip title={'ReadTheDocs'}>
                    <span className="uk-icon-info"></span>
                  </a>
                </li>
              )}
              ...
              {aa && !iframed && (
                <li>
                  <a qkey={"link-to-admin"} title="Settings" href={adminhome}>
                    <i className="uk-icon uk-icon-cog uk-text-middle"></i>
                  </a>
                </li>
              )}
              {aa && !iframed && (
                <li>
                  <a title="Users" href={adminuser}>
                    <i className="uk-icon uk-icon-users uk-text-middle"></i>
                  </a>
                </li>
              )}
              {author && !iframed && (
                <li>
                  <a title="Translations" href={admintranslate}>
                    <i className="uk-icon uk-icon-language uk-text-middle"></i>
                  </a>
                </li>
              )}
		{author && !iframed && (
			...
			<i className="uk-icon uk-icon-user-plus uk-text-middle"></i>
              ...
              <li>
                <a
                  ...
                  data-uk-tooltip
                  title={
                    'Send an email to ' + replyTo + ' and include that this is OpenTA version [' + openTAVersion + ']'
                  }
                >
                  <span className="uk-icon-envelope-o"></span>
                </a>
              </li>
              ...
              {true && (
                <li>
                  {' '}
                  <a title="Logga ut" href={SUBPATH + '/logout/' + course + '/lti_login/'}>
                    {' '}
                    <span className="uk-icon-caret-square-o-right"> </span>
                  </a>
                </li>
              )}
        ...
        {/* Admin menu bar */}
        <div
          className="uk-navbar-content uk-margin-small-top uk-flex uk-flex-middle uk-flex-wrap uk-visible-large"
          style={{ height: 'auto' }}
        >
          {renderPending}
          {canViewXML && <Menu />}
        ...
        {canViewXML && (
          <span>
            <div
              className="uk-navbar-content uk-margin-small-top uk-flex uk-flex-middle uk-flex-wrap uk-hidden-large uk-hidden smallHidden"
              style={{ height: 'auto' }}
            >
              {renderPending}
              {canViewXML && <Menu />}
...
function handleSave(exercise, subdomain) {
  return (dispatch, getState) => {
    var state = getState();
    //console.log("handleSave subdmoain = ", subdomain)
	...
	.then(() => dispatch(fetchExerciseXML(exercise)))
        ...
        .then((res) => dispatch(fetchSameFolder(exercise, state.get('subdomain'))))
	.then(() => dispatch(fetchExerciseRemoteState(exercise )));
    };
...
function handleReset(exercise) {
  // throttleParseXML.cancel(); //Cancel possibly queued XML parsing updates TODO CHECK THIS!
  return (dispatch, getState) => {
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    user_pk: state.getIn(['login', 'user_pk'], 1),
    course: state.getIn(['courses', activeCourse, 'course_name'], ''),
    published: state.getIn(['courses', activeCourse, 'published'], false),
...
const mapDispatchToProps = (dispatch) => ({
  ...
  onSave: (exercise, subdomain) => dispatch(handleSave(exercise, subdomain)),
