...
export const external_renderAsciiMath = (asciitext, thiss) => {
  ...
  try {
    var mParsed = math.parse(parsed).toTex({
      parenthesis: 'keep', // The keep options ksrc/components/questiontypes/renderAsciiMath.jseeps parenthesis from input expression, seems to work best.
    ...
    if (RegExp('fail').test(parsed)) {
      ...
      var outtex = parsed.replace(
        /fail\(\"(.)\"\)/,
        '{\\colsrc/components/questiontypes/renderAsciiMath.jsor{red}{$1} }'
      );
      //syntaxerror = false
    } else if (RegExp('unclosed').test(parsed)) {
      thiss.isUnclosed = false;
      var outtex = parsed.replace('/unclosrc/components/questiontypes/renderAsciiMath.jssed().*/', '');
    }
    var chardict = {
      '%': '\\%',
      '#': 'hash',
