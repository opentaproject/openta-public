...
export default class QuestionMultipleChoice extends Component {
  ...
  constructor(props) {
    super(props);
    try {
    var choices = immutable.fromJS(JSON.parse(this.props.questionState.getIn(['answer'], '{}')))
    } catch(error ){ 
    	var choices = immutable.fromJS(JSON.parse('{}'))
  ...
  dispatchElement = (element) => {
    var itemDispatch = {
      text: (itemjson) => this.props.renderText(itemjson, self.dispatchElement, this.props.lang),
      __text__: (itemjson) => this.props.renderText(itemjson, self.dispatchElement, this.props.lang),
      alt: (itemjson) => this.props.renderText(itemjson, self.dispatchElement, this.props.lang),
  ...
  render() {
    ...
    var lastAnswer = JSON.parse('{}') ; // Last saved answer in database, same format as passed to the submitFunction
        ...
        <div className="uk-width-1-1" key={reactKey}>
          ...
          >
            <div className="uk-panel-badge">
              ...
              {feedback && correctAnswers.get(choiceKey) === false && lastAnswer[choiceKey] === true && (
                <div className="uk-margin-small-left uk-margin-small-right uk-badge uk-badge-danger">
                  <T>Incorrect</T>
                </div>
              )}
              {item.hasIn(['@attr', 'key']) && duplicateKey && (
                <div className="uk-margin-small-right uk-badge uk-badge-danger">Duplicate choice key!</div>
              )}
              {!item.hasIn(['@attr', 'key']) && (
      ...
      <div key={'mulchoice' + nextUnstableKey()} className="">
        ...
        {!this.props.locked && (
          <a
            ...
            className={'uk-width-1-1 uk-button uk-padding-remove uk-button-success uk-margin-bottom click-send '}
          >
