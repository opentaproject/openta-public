...
const BaseComponent = ({auditPk, auditResponseFiles, uploaded, onUpload, uploadPending, uploadProgress, onAuditResponseFileDelete, auditResponseFileDeletePending}) => {
  var renderAuditResponse = auditResponseFiles.map(
    auditResponse => (
      ...
      <div className="exercise-thumb-badge">
        <a onClick={() => UIkit.modal.confirm('Delete response file?', () => onAuditResponseFileDelete(auditPk, auditResponse.get('id')))}><Badge className="uk-badge-notification">
        { !auditResponseFileDeletePending.get(auditResponse.get('id'), false) && <i className="uk-icon uk-icon-trash"/> }
  ...
  var progress = ( 
    ...
    <div className="uk-button-group uk-align-medium-right uk-margin-small"> 
        { uploadPending && <span className="uk-button">{progress} {uploadPending ? (<i className="uk-icon-cog uk-icon-spin"></i>) : ""}</span> }
        ...
        <div className="uk-form-file">
        <a type="file" className={"uk-button"}><i className="uk-icon-file-pdf-o" title="PDF"/>
        </a>
...
const handleAuditResponseFileDelete = (auditPk, auditResponseId) => 
  (dispatch, getState) => {
    var state = getState();
...
const mapStateToProps = state => {
  ...
  return {
    ...
    auditResponseFileDeletePending: state.getIn(['pendingState', 'audit', 'audits', auditPk, 'deletePending'], immutable.Map({})),
  };
}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onAuditResponseFileDelete: (auditPk, auditResponseId) => dispatch(handleAuditResponseFileDelete(auditPk, auditResponseId))
  };
}
