   ...
   if(usethesevars.length > 0 ) {
          ...
          // availableVariables.push( (<span key="s">(i termer av </span>) );
           availableVariables.push( (<span key="s"> <T>in terms of</T> </span>) );
              ...
              var filteredVars = usethesevars.filter(v => typeof v === 'string' && blacklist.indexOf(v) == -1).map( v => v.replace(/\_/g,''));
              for(const [i, v] of filteredVars.entries()) {
                  availableVariables.push((<span key={"v"+i}>{v}</span>));
                  if(varProps.hasIn([v, 'tex']))
                      availableVariables.push((<span key={"tex"+i}> (<MathSpan message={"$" + varProps.getIn([v,'tex']) + "$"}></MathSpan>)</span>));
