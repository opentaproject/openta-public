...
return (
  <li key={exercise} id={exercise} className="course-exercise-item " >
    ...
    <a className={"uk-thumbnail exercise-a " + (meta.published ? "" : "exercise-unpublished")} onClick={(ev) => onExerciseClick(exercise, foldername, folder.get('path'), exerciseState.getIn([exercise, 'selected']))} style={selectedStyle}>
        <div className="exercise-thumb-wrap" style={{minWidth: '80px'}}>
            ...
            <div className="exercise-thumb-badge">
                ...
                { meta.image && <span className={"uk-badge uk-badge-notification "}><i className="uk-icon uk-icon-camera"/></span> }
...
const BaseExerciseSelect = ({ exercisetree, exerciseTreeUI, exerciseState, pendingState, currentpath, onExerciseClick, showStatistics, statistics, activityRange, onFolderClick, student, author }) => {

  function parseFolder( folder, foldername, level=0 ) {
    var exercises = immutable.List([]), children = [];
    if(folder.has('exercises')) {
      exercises = folder.get('order')
