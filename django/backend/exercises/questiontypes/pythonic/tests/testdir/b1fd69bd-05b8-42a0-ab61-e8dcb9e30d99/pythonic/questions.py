...
# Taylor clashes
ns.update( {
        'C': C, 'zeta': zeta,  'N': N, 'Q': Q,  'O': O, 'beta': beta, 'S': S, 'gamma': gamma, 
        'ff': Symbol('ff'),
        'FF': Symbol('FF')
        })
    ...
    try:
        ...
        for  csvfile in csvfiles :
            ...
            if fileexists:
                ...
                response['warning'] = 'A2: file ' + os.path.basename( filename ) + ' does not exist'
                return response
        res =  getOmegas( csvars, trainingSet) 
        if res['COmega'] < .01 and res['HOmega'] < .02  :
