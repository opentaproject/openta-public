...
class legacy_linearAlgebraUnitError(Exception):
    """
...
def check_units_new(expression, correct, sample_variables):
    ...
    Returns:
        Nothing
    Raises:
        legacy_linearAlgebraUnitError if the units do not match
    """
    nvarsubs = {}
    nsubs_values = []
    ...
    for res in results:
        if numpy.absolute(res - results[0]) > 10e-5:
            raise legacy_linearAlgebraUnitError(
                _("Seems like the expression does not have the correct units.")
            )


def legacy_linear_algebra_compare_expressions(
    ...
    return legacy_linear_algebra_check_equality(lhs, rhs, sample_variables, check_units=check_units)


def legacy_linear_algebra_check_equality(lhs, rhs, sample_variables, check_units=True):  # {{{
    """
    ...
    try:
        ...
        if check_units:
            ...
            except legacy_linearAlgebraUnitError as e:
...
def legacy_linear_algebra_expression_runner(
    variables, expression1, expression2, check_units, blacklist, result_queue
):
    response = legacy_linear_algebra_compare_expressions(
        variables, expression1, expression2, check_units, blacklist
    )
    result_queue.put(response)


def legacy_linear_algebra_expression(
    ...
    return safe_run(
        legacy_linear_algebra_expression_runner,
        args=(variables, student_answer, correct_answer, check_units, blacklist),
    )


def legacy_linear_algebra_expression_blocking(
    variables, student_answer, correct_answer, check_units=True, blacklist=[]
):
    """
    Starts a process with compare_numeric_internal that will be terminated if it takes too long. This implementation uses multiprocessing.Process.
    """
    return legacy_linear_algebra_compare_expressions(
