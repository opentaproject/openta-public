...
def create_xlsx_from_results_list(results):
    ...
    worksheet.write(0, 10, 'Passed audits')
    worksheet.write(0, 11, 'Total audits')
    worksheet.write(0, 12, 'Manually passed')
    worksheet.write(0, 13, 'Total (Correct exercises)')
    for index, student in enumerate(results):
        ...
        worksheet.write(index + 1, 11, student['total_audits'])
        worksheet.write(index + 1, 12, student['manually_passed'])
        worksheet.write(index + 1, 13, student['total'])
    workbook.close()
    output.seek(0)
    return output.read()
