...
DEFAULT_EXERCISE_NAME = "Export-Import"
DEFAULT_EXERCISE_TEMPLATE = """
                ...
                """
DATABASE_PASSWORD = 'learning'
DEFAULT_EXERCISE = DEFAULT_EXERCISE_TEMPLATE.format(name=DEFAULT_EXERCISE_NAME)


def create_database(password="learning", course_key=None, course_name='Test course'):
    if course_key is not None:
        course, created = Course.objects.get_or_create(
            course_name=course_name, course_key=course_key, published=True
        )
    else:
    ...
    uadmin = User.objects.create_superuser('admin1', 'admin1@test.se', 'learning')
    usuper = User.objects.create_superuser('super', 'admin1@test.se', 'learning')
...
def create_database789(password="learning", course_key=None, course_name='Test course'):
    if course_key is not None:
        course, created = Course.objects.get_or_create(
            course_name=course_name, course_key=course_key, published=True
        )
    ...
    u1 = create_user('student1', 'student1@test.se', 'learning', course=course)
    u2 = create_user('student2', 'student2@test.se', 'learning', course=course)
    u3 = create_user('student3', 'student3@test.se', 'learning', course=course)
    uadmin = User.objects.create_superuser('admin1', 'admin1@test.se', 'learning')
    usuper = User.objects.create_superuser('super', 'admin1@test.se', 'learning')
    student.user_set.add(u1)
    student.user_set.add(u2)
