...
def create_instance(args):
    instances.create_instance(args.output_path, args.subpath, args.docker_repo, args.version)
...
def build_current_version(args):
    run_and_inspect('make build-deploy-docker', env=dict(DOCKER_REPO=args.docker_repo), cwd="../../", shell=True)
...
def parse_arguments():
    ...
    version = subprocess.check_output(["make", "version"], cwd="../../").decode("utf-8").strip("\n")
    ...
    create_instance_parser.add_argument("--version", type=str, default=version)
    ...
    migrate_instance_parser = subparsers.add_parser("migrate-instance")
    migrate_instance_parser.set_defaults(func=migrate_instance)
    migrate_instance_parser.add_argument("name", type=str)
    ...
    build_parser = subparsers.add_parser("build-openta")
    build_parser.set_defaults(func=build_current_version)
    build_parser.add_argument("docker_repo", type=str)
