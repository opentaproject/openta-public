...
kubectl create namespace ${DOMAIN_NAME}-${DOMAIN}
...
helm repo update
helm install ingress-nginx ingress-nginx/ingress-nginx  --set rbac.create=true  --namespace ${DOMAIN_NAME}-${DOMAIN}   --set controller.service.loadBalancerIP=${IP}
...
kubectl get service ingress-nginx-controller --namespace=${DOMAIN_NAME}-${DOMAIN}
helm repo update 
kubectl create namespace cert-manager
helm repo add jetstack https://charts.jetstack.io
helm repo update
helm install cert-manager jetstack/cert-manager --namespace cert-manager --version v1.1.0  --set installCRDs=true
kubectl create secret generic clouddns-service-account-${DOMAIN_NAME}-${DOMAIN}  \
	--from-file=service-account-key.json=${KEYFILE} \
	--namespace=${DOMAIN_NAME}-${DOMAIN}
gcloud iam service-accounts create dns01-solver-${DOMAIN_NAME}-${DOMAIN}
gcloud iam service-accounts keys create key.json --iam-account dns01-solver-${DOMAIN_NAME}-${DOMAIN}@${PROJECT_ID}.iam.gserviceaccount.com
k apply -f yamls-staging
kubectl config set-context --current --namespace=${DOMAIN_NAME}-${DOMAIN}
...
curl https://${DOMAIN_NAME}.${DOMAIN}  -k 
